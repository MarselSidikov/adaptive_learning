...
CREATE TABLE Students (id INT , first_name VARCHAR(30), last_name VARCHAR(30), login VARCHAR(30), pass_hash VARCHAR(20), current_lesson INT,userToken Varchar(100), current_urls VARCHAR(1000), PRIMARY KEY (id), FOREIGN KEY (current_lesson)  REFERENCES Lessons(id));
CREATE TABLE Questions (id INT , question VARCHAR(100), correctAnswerId INT, block INT, PRIMARY KEY (id));
CREATE TABLE Lessons (id INT , mainMatUrl VARCHAR(100), extra_material_url VARCHAR (100), block_urls VARCHAR (1000), block_test_ids INT, main_test_ids, next_lesson INT, PRIMARY KEY (id),FOREIGN KEY (main_test_id) REFERENCES Test (id),FOREIGN KEY (block_test_id) REFERENCES Test(id));
